...
input_data = rece_data_ofdm_bench;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% STEP 1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Passband filtering
Y_pb = bandpass(input_data,[-4000+Fc,4000+Fc],sampling_rate);
figure()
plot(Y_pb)
title('OG sample')
...
a = v/c;
T_tx = 8.26952; % Transmitted signal duration

% factor_a_hat = 1.0001871;
% absrd = factor_a_hat*2116495;
% T_rx = (1/sampling_rate)*round(absrd); % Ask TA how to calculate this value.
T_rx = (1/sampling_rate)*(2116925 - 25);
% T_rx = (1/sampling_rate)*(2116840);
a_hat = T_tx/T_rx -1;
% a_hat = 5e-5;
...
%     Y_BB_Q(n) = -1*Y_pb_re_mf_tilde(n)*2*sin(2*pi*Fc*n*ts);
% end
...
% n_start = 2200 + ninx;
n_start = start_pos(ninx);
disp(n_start)
% eps_start = -2 + epsinx*0.1;
eps_start = epsilon(epsinx);
disp(eps_start)
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%% STEP 10 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Grid Search for all subcarriers/OFDM Symbols and Fine tuning
num_OFDM_symbols = 21;
...
% n_start_W = n_start;
fine_Tune_Range = [-2*lambda:1:2*lambda];
for W = 2:num_OFDM_symbols
    clear("P_null")
    ninx = 0;
    for n_0w = symbol_start_pos(W-1)+(K+L)*lambda+fine_Tune_Range
        ninx = ninx+1;
        epsinx = 0;
        for eps_inx = 1:length(epsilon)
...
symbol_start_pos(W) = symbol_start_pos(W-1)+(K+L)*lambda + fine_Tune_Range(ninx);
...
z_diff = bb_rece_data_174623(1:2048,:) - Z_values;
check_valid = abs(z_diff) > ones(size(z_diff)).*1e-7;
valid_sum = sum(sum(check_valid));
